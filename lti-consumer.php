...
function sb_lti_launch_func($attrs) {
    ...
    if ( array_key_exists('error', $data) ) {
        ...
        $html .= "<form method=\"post\" onsubmit=\"setTimeout(function(){window.location.href += \"#mypara\";window.location.reload();},3)\" action=\"$surl\"  target=\"$target\" id=\"launch-$id\" data-id=\"$id\" data-post=\"$data[id]\" data-auto-launch=\"$autolaunch\">";
        ...
        if ( $data['display'] == 'iframe' ) {
            ...
            $html .= '<button onclick="lti_consumer_launch(\'' . $id . '\'); sb_lti_launch_func($attrs);">Launch ' . $data['text'] . '</button>';
        }
